...
params = pd.read_csv('cldf_test/etc/parameters.tsv', sep = ',', index_col = 0)
years = pd.read_csv('cldf_test/raw/others/lang_sources.tsv', sep = ',', index_col = 0)
data = pd.DataFrame()
...
data = data.rename(columns={"CARACTERISTICA":"Feature",
                        "Estado 1":"State_1",
...
new_data = pd.merge(data, lookup_prm, left_on = data['Feature'].str.lower(), right_on = lookup_prm['Description_esp'].str.lower(), how = 'left')
...
def split_obs(df):
    state_1 = df.drop(columns = df.columns[[6, 7, 8, 9, -1]])
    state_1 = pd.merge(state_1, years, left_on = 'Language', right_on = 'ID', how = 'left')
    state_1 = state_1.drop(columns = state_1.columns[[-1, -3, -5]])
    state_1.columns = state_1.columns.str.replace(pat='_1', repl='', regex = True)

    state_2 = df.drop(columns = df.columns[[2, 3, 4, 5]])
    state_2 = pd.merge(state_2, years, left_on = 'Language', right_on = 'ID', how = 'left')
    ...
    full_order = full.sort_values(by = ['Language', 'Parameter', 'Value'], ignore_index=True)
